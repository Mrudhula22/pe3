...
def generate_churn_data(path: Path, rows: int = 1600):
    rng = np.random.default_rng(42)
    data = pd.DataFrame({
        'tenure': rng.integers(1, 72, rows),
        'monthly_charges': rng.uniform(20, 120, rows),
        'total_charges': rng.uniform(500, 6000, rows),
        'contract_type': rng.integers(0, 3, rows),
        'support_calls': rng.integers(0, 10, rows),
        'internet_service': rng.integers(0, 3, rows),
        'is_premium': rng.integers(0, 2, rows),
    })
    ...
    churn_score = (
        ...
        + 0.001 * data['total_charges']
        + 0.6 * data['contract_type']
        + 0.35 * data['support_calls']
        + 0.9 * data['internet_service']
        + 0.8 * data['is_premium']
    )
    prob = 1 / (1 + np.exp(-churn_score))
    data['churn'] = (rng.random(rows) < prob).astype(int)
...
def load_or_generate_data():
    ...
    return generate_churn_data(file_path)
